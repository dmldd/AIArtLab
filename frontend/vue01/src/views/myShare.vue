    ...
    <div>
        ...
        <div ref="masonry" class="masonry">
            <div v-for="(item, index) in allSharePic" :key="index" class="masonry-item">
                <img :src="item.url" :alt="item.nickname">
                <div class="info-overlay"
                    style=" color:hsla(0,0%,100%,.8);   font-weight: 500;font-size: 15px;">
                    <i class="el-icon-star-on" style="font-size: 25px;"></i> 
                    <span style="font-size: 20px;">{{ item.store }}</span>
                </div>
                ...
                <el-button @click="showPicMes(item)" class="param-button"
                    style="background-color: transparent;color:hsla(0,0%,100%,.8) ;border: 0px;"> <el-icon name="view"
                        class="view-icon"></el-icon> 图片参数
                </el-button>

                <span>ID:{{ item.picId }}</span>
