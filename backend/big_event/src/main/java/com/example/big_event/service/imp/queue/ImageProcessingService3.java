...
@Service
public class ImageProcessingService3 {
    private static final Integer QUEUE_NUM=3;
    @Autowired
    ...
    public void processQueue() {
        ...
        new Thread(() -> {
            try {
                while (!queueService.isEmpty(3)) {
                    if (currentProcessingCount.get() < maxQueueSize) {
                        QueueWrapper requestWrapper = queueService.dequeue(3);
    ...
    private void processRequest(QueueWrapper requestWrapper) {
        ...
        String url = userServiceImp.getSdUrl(3) + "/sdapi/v1/txt2img";
        StableDiffusionTextToImg stableDiffusionTextToImg = requestWrapper.getTextToImgIfo();
        try {
            // 更改状态
            requestStatusService.updateRequestStatus(requestId, currentProcessingCount.get(), 3);
            // 向API发送请求，并接收响应
            ...
            // 更新请求状态为-1（处理完成）
            requestStatusService.updateRequestStatus(requestId, -1, 3);  //-1表示生成图片成功
        } catch (Exception e) {
            System.out.println("Error while processing request " + requestId + ": " + e.getMessage());
            // 在出现异常时更新请求状态
            requestStatusService.updateRequestStatus(requestId, -2, 3); // 假设-2表示出错
        } finally {
            currentProcessingCount.decrementAndGet(); // 减少正在处理的请求计求
            requestStatusService.updataAllStatus(3);
            System.out.println(requestId + ":" + requestStatusService.getRequestStatus(requestId, 3));
        }
