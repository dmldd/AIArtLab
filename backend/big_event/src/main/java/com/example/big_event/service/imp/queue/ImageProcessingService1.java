...
@Service
public class ImageProcessingService1 {
    private static  final Integer  QUEUE_NUM=1;
    @Autowired
    ...
    private void processRequest(QueueWrapper requestWrapper) {
        ...
        //Tagger
        else if(requestWrapper.getTaggerIfo() != null) {
            Tagger taggerIfo = requestWrapper.getTaggerIfo();
            String url = userServiceImp.getSdUrl(1) + "/tagger/v1/interrogate";
            try {
                // 更改状态
                requestStatusService.updateRequestStatus(requestId, currentProcessingCount.get(), 1);
                // 向API发送请求，并接收响应
                String response2 = SendToApi.sendToApi(url, null, taggerIfo, null);
                //将json里的prompt提取出来：
                ...
                // 将生成的图片Tagger放入urlMap
                Return_pic return_pic=new Return_pic(result,false);
                requestUrlService.createRequestUrl(requestId, return_pic);
                // 更新请求状态为-1（处理完成）
                requestStatusService.updateRequestStatus(requestId, -1, 1);
            } catch (Exception e) {
                System.out.println("Error while processing request " + requestId + ": " + e.getMessage());
                // 在出现异常时更新请求状态
                requestStatusService.updateRequestStatus(requestId, -2, 1); // 假设-2表示出错
            } finally {
                currentProcessingCount.decrementAndGet(); // 减少正在处理的请求计求
                requestStatusService.updataAllStatus(1);
            }
        }
