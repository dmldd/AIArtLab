...
@Service
public class ImageProcessingService4 {
    private static final Integer QUEUE_NUM=4;
    @Autowired
    ...
    public void processQueue() {
        ...
        new Thread(() -> {
            try {
                while (!queueService.isEmpty(4)) {
                    if (currentProcessingCount.get() < maxQueueSize) {
                        QueueWrapper requestWrapper = queueService.dequeue(4);
    ...
    private void processRequest(QueueWrapper requestWrapper) {
        ...
        if (requestWrapper.getTextToImgIfo() != null) {
            String url = userServiceImp.getSdUrl(4) + "/sdapi/v1/txt2img";
            StableDiffusionTextToImg stableDiffusionTextToImg = requestWrapper.getTextToImgIfo();
            try {
                // 更改状态
                requestStatusService.updateRequestStatus(requestId, currentProcessingCount.get(), 4);
                // 向API发送请求，并接收响应
                ...
                // 更新请求状态为-1（处理完成）
                requestStatusService.updateRequestStatus(requestId, -1, 4);
            } catch (Exception e) {

                // 在出现异常时更新请求状态
                requestStatusService.updateRequestStatus(requestId, -2, 4); // 假设-2表示出错
            } finally {
                currentProcessingCount.decrementAndGet(); // 减少正在处理的请求计求
                requestStatusService.updataAllStatus(4);
            }
        }
        //图生图
        else {
